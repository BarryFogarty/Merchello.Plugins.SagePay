    ...
    public class SagePayPaymentProcessor
    {
        ...
        public IPaymentResult InitializePayment(IInvoice invoice, IPayment payment, ProcessorArgumentCollection args)
        {
            ...
            try
            {
                ...
                // 1. Gather SagePay settings info and formulate it into a Dictionary for posting to the gateway
                var sagePayFormIntegration = new SagePayFormIntegration(_settings);
                ...
                // 2. use the SagePay methods to encrypt the form so it can be posted

                var content =
                        new FormUrlEncodedContent(
                            new Dictionary<string, string>
						{
							{"VPSProtocol", _settings.ApiVersion},
                            {"TxType", Constants.TransactionType},
                            {"Vendor", _settings.VendorName},
                            {"Crypt", request.Crypt}
                ...
                // 3. Post the form to SagePay VSP
                var formPaymentUrl = string.Format("https://{0}.sagepay.com/gateway/service/vspform-register.vsp", GetModeString(_settings.LiveMode));
                ...
                // 4. Process the response from SagePay - this contains the redirect URL for the customer to complete their payment.
        ...
        private void SetSagePayApiData(IFormPayment request, IInvoice invoice, IPayment payment)
        {
            ...
            request.VpsProtocol = _settings.ProtocolVersion;
            request.TransactionType = _settings.TransactionType;
            request.Vendor = _settings.VendorName;
            ...
            // TODO:  Is there a basket summary I can access?
            request.Description = "Goods from " + _settings.VendorName;
            request.SuccessUrl = _settings.SuccessCallbackUrl;
            request.FailureUrl = _settings.FailureCallbackUrl;

            // TODO:  Pass different details for shipping/billing name
            ...
            //request.VendorEmail = _settings.VendorEmail;
            //request.SendEmail = _settings.SendEmail;

            //request.EmailMessage = _settings.EmailMessage;
            //request.BillingAddress2 = billingAddress.Address2;
            //request.BillingPostCode = billingAddress.PostalCode;
